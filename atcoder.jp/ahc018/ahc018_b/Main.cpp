...
struct Solver {
    ...
    vector<pair<Pos, Pos>> make_MST(vector<Pos>& object) {
        ...
        for (int i = 0; i < M; i++) {
            for (int j = i+1; j < M; j++) {
                if (i < W && j < W) continue;
                edge.push_back({calc_dist(object[i], object[j]), {i, j}});
            }
        }
        ...
        for (int i = 0; i < (int)edge.size(); i++) {
            ...
            if (!uf.same(u, v)) {
                ...
                if (dist_m <= 50) {
                    MST_edge.push_back({object[u], object[v]});
                    continue;
                }
                ...
                if (dist_m <= 100) {
                    MST_edge.push_back({object[u], rp});
                    MST_edge.push_back({rp, object[v]});
                    continue;
                }

                Pos rp2 = make_relaypoint(object[u], rp);
                Pos rp3 = make_relaypoint(rp, object[v]);
                MST_edge.push_back({object[u], rp2});
                //MST_edge.push_back({rp2, rp});
                MST_edge.push_back({rp2, rp3});
                //MST_edge.push_back({rp, rp3});
                MST_edge.push_back({rp3, object[v]});
            }
        }
    ...
    vector<pair<Pos, Pos>> make_graph(vector<Pos>& object) {
        ...
        for (int i = 0; i < node_cnt; i++) {
            for (int j = i+1; j < node_cnt; j++) {
                if (i < W && j < W) continue;
                edge.push_back({calc_dist_m(node[i], node[j]), {i, j}});
            }
        }
        sort(edge.begin(), edge.end());        
        ...
        while (connect_cnt < (int)house_pos.size()) {
            ...
            for (int os = 0; os < M; os++) {
                ...
                for (int og = 0; og < M; og++) {
                    ...
                    object_dist.push_back({calc_dist_m(node[os], node[og]), {os, og}});
                } 
            }
            ...
            while(cv != from) {
                for (int nv: graph[cv]) {
                    if (cost[nv] + calc_dist_m(node[cv], node[nv]) == cost[cv]) {
                        graph_edge.push_back({node[cv], node[nv]});
                        cv = nv;
                        break;
                    }
                }
